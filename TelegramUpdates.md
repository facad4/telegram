...
- **Environment**: `.env` file for secrets (`SUPABASE_URL`, `SUPABASE_KEY`, `TELEGRAM_API_ID`, `TELEGRAM_API_HASH`, `TELEGRAM_SESSION`, `GROK_API_KEY`, `GOOGLE_API_KEY`, `MISTRAL_API_KEY`, `TAVILY_API_KEY`, `NVIDIA_API_KEY`, `NIM_UNIFEED`, `OPENROUTER_API_KEY`, `TELEGRAM_BOT_TOKEN`, `DIGEST_SERVER_URL`, `DIGEST_TOC_KEY`, `DIGEST_TODAY_TZ`) loaded via `python-dotenv`. `DIGEST_SERVER_URL` is the base URL the server uses to build the headlines-button link on bot-posted messages; `DIGEST_TOC_KEY` is the secret token gating `GET /digest/today` (also required on the digest host to build the button URL); `DIGEST_TODAY_TZ` sets the "today" boundary for that page (default `Asia/Jerusalem`). The digest host additionally reads `DIGEST_DEV_SERVER_URL` (fetch target under `--test`) and `DIGEST_TOC_SERVER_URL` (fixed headlines-button host) — see Standalone Scripts.
...
- **Settings**:
  ...
  - `hostile_framing_screen_enabled`: When true (default), the digest script holds back any story that reproduces hostile-media framing without attribution (see `alternate_feed_prompt.md` section 1A); held stories are logged to `debug.log` (`test_debug.log` under `--test`) for manual review instead of being posted
...
A CLI tool that fetches posts from the alternate feed, processes them through a configurable AI provider (Mistral or NVIDIA NIM), deduplicates stories against history, generates a standalone HTML digest file, and optionally posts new stories to a Telegram channel via a bot with two inline buttons (Perplexity enrichment + "הכותרות של היום" today's-headlines). Maintains persistent history in `digest_history.json` for incremental updates across runs.
...
- `--server URL` — Production server base URL (default: `DIGEST_SERVER_URL` env var). Used for fetch/login/media when not `--test`, and — unless `--toc-server` / `DIGEST_TOC_SERVER_URL` is set — as the host the headlines button links to
- `--dev-server URL` — Dev server base URL to fetch/login/resolve-media from when `--test` is set (default: `DIGEST_DEV_SERVER_URL` env var); falls back to `--server` when unset
- `--toc-server URL` — Public server base URL the "הכותרות של היום" button links to, regardless of `--test` (default: `DIGEST_TOC_SERVER_URL` env var); falls back to `--server` when unset. Set this when the digest is run against a LAN/localhost server but the button must point at the deployed instance
...
- `--output FILE` — Output HTML file path (default: `alternate_digest.html`; `test_alternate_digest.html` under `--test`)
- `--no-telegram` — Skip Telegram posting entirely
- `--test` — Test mode: uses `test_digest_history.json`, `test_debug.log`, `TEST_DIGEST_TELEGRAM_CHANNEL`, `--dev-server` for fetching, and `ch=test` on the headlines button
- `--single` — Skip AI/dedup/history; post only the first fetched post (raw) and exit. Targets `DIGEST_TELEGRAM_CHANNEL`, or `TEST_DIGEST_TELEGRAM_CHANNEL` with `--test`. Does **not** save to history, so a later normal run may regenerate a story from the same source post

**Environment variables** (from `.env` or shell):
- `DIGEST_SERVER_URL` – production server base URL (e.g. `https://app.onrender.com`); auto-prepends `http://` if no protocol is specified. Used for fetch/login/media when not `--test`, and as the headlines-button host unless `DIGEST_TOC_SERVER_URL` is set
- `DIGEST_DEV_SERVER_URL` – dev server base URL fetched from when `--test` is set (optional; falls back to `DIGEST_SERVER_URL`)
- `DIGEST_TOC_SERVER_URL` – public server base URL the "הכותרות של היום" button always links to (optional; falls back to `DIGEST_SERVER_URL`). Needed when the digest runs against a LAN/localhost server
- `DIGEST_TOC_KEY` – secret token appended to the headlines-button URL (`?k=`); must match the value configured on the server serving `GET /digest/today`. The button is only attached when this **and** a headlines-button host are set
...
- `TELEGRAM_BOT_TOKEN` – Telegram bot token (optional); if set, stories are posted via a bot client which adds two inline buttons — "ספר לי עוד על זה" (Perplexity enrichment) and "הכותרות של היום" (today's headlines, when a headlines-button host + `DIGEST_TOC_KEY` are set); falls back to user client if missing. The bot client uses a persistent `digest_bot.session` file (not an ephemeral `StringSession`) to avoid re-running bot authorization every run and tripping Telegram's flood limit
...
**Pipeline**:
1. **Authentication**: Logs in via `POST /api/login` against the fetch server (dev server under `--test` when `DIGEST_DEV_SERVER_URL`/`--dev-server` is set, otherwise `DIGEST_SERVER_URL`) using admin credentials; verifies admin status
...
7A. **Grammar & Framing Passes**: `grammar_check_stories()` runs a Hebrew copy-edit model (`grammar_checker.md`, provider from `grammar_checker_*` config keys) over each story — language/gender/phrasing fixes only, never meaning; it preserves `> ` quote lines. Then `screen_hostile_framing()` (config `hostile_framing_screen_enabled`, default true) enforces `alternate_feed_prompt.md` section 1A: if a story is built from a source post that only quotes/translates hostile media (signals: "ערוצים פלסטינים", "תרגום:" + an Arabic block, etc.) and its bold line does not attribute the claim — or the text uses "אלבראק" with no attribution anywhere — the story is **held**: dropped from the run (not posted, not saved to history) and dumped to `debug.log` (`test_debug.log` under `--test`) under a `HOSTILE-FRAMING REVIEW` header for manual review

8. **Media URL Resolution**: Converts relative `/api/` media URLs to absolute URLs using the **fetch** server's base URL (the dev server under `--test`, otherwise `DIGEST_SERVER_URL` — never the headlines-button host); includes `photo_url`, `video_thumb`, and `link_preview.image` fallbacks. Uses URL-path fingerprinting (Layer 1 dedup) to eliminate CDN variants of the same image before downloading; intra-story and cross-story duplicate image removal (Layers 1.5 and 1.75). A pre-scan of video thumbnail URLs builds a fingerprint set before processing `media_urls`, preventing video thumbnails from being duplicated in `media_urls` (they are kept exclusively in `video_thumb_urls`)
...
- `screen_hostile_framing(stories, original_posts) -> list[dict]`: Enforces `alternate_feed_prompt.md` section 1A. Drops (holds) any story whose source post only relays/translates hostile media (`_is_hostile_media_quote()`) when the story's bold line does not attribute the claim, or whose text uses "אלבראק" with no attribution anywhere; held stories are logged and written to `DEBUG_LOG_PATH` (`debug.log`, or `test_debug.log` under `--test`) via `_write_hostile_framing_review()`. Returns the stories cleared to proceed. Gated by config `hostile_framing_screen_enabled` (default true)
- `_md_to_html(text) -> str`: Converts the Markdown subset used in story text to Telegram HTML — `**bold**` → `<b>`, and consecutive `> `-prefixed lines → a single `<blockquote>` (used for attributed hostile-media quotes)
...
- `connect_telegram_bot() -> TelegramClient | None`: Creates and connects a Telethon bot client using `TELEGRAM_BOT_TOKEN`, backed by a persistent `digest_bot.session` file (reuses the prior bot authorization instead of re-running it every run); returns `None` if token is missing
- `post_stories_to_telegram(stories, channel, posted_media_hashes, new_story_target=None, toc_url=None) -> set[str]`: Posts stories to a Telegram channel; uses bot client if available, falls back to user client. When on the bot client, attaches the Perplexity inline button plus — when `toc_url` is passed — the "הכותרות של היום" URL button. Content-hash media dedup (Layers 2+3); downloads images and videos into memory, sends photo albums then individual videos with streaming support; returns updated hash set
...
System prompt used by the standalone digest script for Mistral AI processing. Contains these sections:

0. **Pre-Filter — Satire & Parody**: discard satirical/parody posts before ranking
1. **Core Task**: Operational steps — deduplicate, select (up to `{max_stories}` stories, substituted at runtime), merge same-event posts, rephrase, strip source attribution (Telegram channel names only — media attribution *inside* a post is preserved), enforce temporal accuracy, topical coherence, source constraint, and media preservation
1A. **Reports That Quote or Translate Other Media**: when a source post merely relays/translates another outlet (signals: "ערוצים פלסטינים:", "תרגום:", an Arabic block + Hebrew translation, hostile framing like "אלבראק"/"פרצו"/"מתנחלים" for worshippers), the story must NOT adopt that framing as the digest's own voice. Instead: (1) the bold subject line attributes the claim ("תקשורת פלסטינית: …"), (2) the quoted claim goes in a `> ` quote block introduced by an attribution phrase, with a one-time parenthetical gloss for hostile place-names (רחבת "אלבראק" (הכותל המערבי)), (3) one digest-voice sentence gives the real context/Israeli framing, (4) hostile characterizations appear ONLY inside the `> ` quote. Includes a neutral-terms glossary (הכותל המערבי, הר הבית, יהודים/מתפללים, עלו/נכנסו) for the digest's own voice
...
6. **Final Verification** (mandatory before returning): self-dedup, topical-coherence check, source audit (every output item must have valid `source_indices` from `new_posts`), final count cap (at most `{max_stories}`), and hostile-media attribution check (section 1A)
...
A second inline URL button attached alongside the Perplexity button on every bot-sent digest channel post (`generate_alternate_digest.py`, `test_generate_alternate_digest.py`, `POST /api/admin/compose-to-channel`, `POST /api/admin/upload-image-to-channel`). It opens `GET /digest/today` (see API Endpoints) in Telegram's in-app browser — a scrollable RTL list of today's headlines (light "paper white" theme by default, with a dark-mode toggle) that deep-link back into the channel; updates are grouped under their original story with a "הצג עדכונים" popup. Nothing is posted to the channel when tapped. The button is added only when the bot client is available **and** `DIGEST_TOC_KEY` plus a headlines-button host are set — for the digest scripts the host is `DIGEST_TOC_SERVER_URL` (falling back to `DIGEST_SERVER_URL`), for the server endpoints it is `DIGEST_SERVER_URL`. The same `DIGEST_TOC_KEY` value must be configured on the host serving `GET /digest/today` and on the digest host. In the digest scripts the button URL always points at this fixed host regardless of `--test` (only `ch=` switches: `test` under `--test`, `prod` otherwise), so a digest run against a LAN/localhost fetch server still produces a publicly-openable button.
...
├── alternate_feed_prompt.md   # System prompt for alternate feed digest AI processing (dedup, rank, rephrase, Proud Patriot persona, hostile-media quote attribution — section 1A)
├── grammar_checker.md         # System prompt for the Hebrew copy-edit pass over generated digest stories (language/gender/phrasing only; preserves > quote blocks)
...
├── digest_history.json    # Persistent storage for generated stories, processed post keys, post texts, and posted media hashes (created/updated by generate_alternate_digest.py; test_digest_history.json under --test)
├── test_nim_key_state.json  # Tracks last-used NIM API key index for rotation between NVIDIA_API_KEY/NIM_UNIFEED (shared by generate_alternate_digest.py and test_generate_alternate_digest.py — not switched by --test)
├── debug.log              # Hostile-framing review dump + digest debug output (test_debug.log under --test); gitignored
├── digest_bot.session     # Persistent Telethon bot session reused across digest runs (gitignored)
...
├── .env                   # Environment variables (SUPABASE_URL, SUPABASE_KEY, TELEGRAM_*, GROK_API_KEY, GOOGLE_API_KEY, MISTRAL_API_KEY, NVIDIA_API_KEY, OPENROUTER_API_KEY, TAVILY_API_KEY, TELEGRAM_BOT_TOKEN, DIGEST_SERVER_URL, DIGEST_DEV_SERVER_URL, DIGEST_TOC_SERVER_URL, DIGEST_USERNAME, DIGEST_PASSWORD, DIGEST_TELEGRAM_CHANNEL, TEST_DIGEST_TELEGRAM_CHANNEL, DIGEST_TOC_KEY, DIGEST_TODAY_TZ, PX_MAX_CONCURRENCY, PX_COOLDOWN_S)
